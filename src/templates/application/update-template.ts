import { inject, injectable } from 'inversify';
import { TYPES as SHARED_TYPES } from '../../shared/di/index.js';
import type { Clock } from '../../shared/clock/index.js';
import { TEMPLATE_TYPES } from '../types.js';
import { Template, type TemplateId } from '../domain/template.js';
import { TemplateNotFoundError } from '../domain/errors.js';
import type { TemplateRepository } from './template-repository.js';
import type { TemplateRenderer } from './template-renderer.js';
import type { UpdateTemplateInput } from './dtos.js';

/**
 * Applies a partial change set to an existing template, re-confirms the
 * (possibly changed) bodies still compile against the bound renderer, and
 * persists it.
 */
@injectable()
export class UpdateTemplate {
  constructor(
    @inject(TEMPLATE_TYPES.TemplateRepository)
    private readonly repository: TemplateRepository,
    @inject(TEMPLATE_TYPES.TemplateRenderer)
    private readonly renderer: TemplateRenderer,
    @inject(SHARED_TYPES.Clock) private readonly clock: Clock,
  ) {}

  async execute(id: TemplateId, changes: UpdateTemplateInput): Promise<Template> {
    const template = await this.repository.findById(id);
    if (template === null) {
      throw new TemplateNotFoundError(id);
    }

    template.update(changes, this.clock.now());
    this.renderer.render(template, {});

    await this.repository.update(template);
    return template;
  }
}
