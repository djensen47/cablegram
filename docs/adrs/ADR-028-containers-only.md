# ADR-028: Containers Only — retiring the serverless function target
...
A note on naming: this ADR says *serverless function* where the point is general and names
**DigitalOcean Functions** only where the fact is specific to it. That distinction is load-bearing —
the limitation that decided this (§Context 3) is DO's, not serverless computing's, so a reader must
not come away thinking "functions can't reach a private database".

## Context

[ADR-009](ADR-009-deployment-digitalocean-functions.md) picked a serverless function — DigitalOcean
Functions specifically — as the initial deployment target, with Docker as "an eventual goal", and
told the code to *design to the stricter runtime*: stateless, ephemeral, no background workers, no long in-request loops, no local disk,
...
3. **It cannot reach the database.** A DigitalOcean Functions component cannot join a VPC, so it
   cannot reach a privately-addressed MongoDB (issue #44). This is not a packaging problem with a
   workaround — it is the runtime being unable to talk to the only durable state the app has.

(3) is decisive on its own, and it is the one fact here that is genuinely DigitalOcean's rather than
serverless computing's — other providers' functions do attach to a private network. That matters for
how far this ADR reaches: it retires **the serverless target we had**, on the evidence we have. It is
not a finding that functions are the wrong shape for this app.

What remained was an unverified config file and a 90-line adapter published as `cablegram/function`,
maintained for a target nothing could deploy to.
...
in the documentation*: ADR-009's constraints are cited across the codebase as "because the function
runtime says so", and a reader who checks discovers the justification names a target that cannot be
used. That teaches the constraints are obsolete, which is the opposite of true. A published `./function` export
is worse than dead code — it is a **promise**, and promises are versioned (ADR-026).

## Decision

**cablegram deploys as a long-running container. The serverless function target is retired.**

### 1. Deleted

- `src/function.ts` — the adapter for OpenWhisk's raw web-action contract, which is what DO Functions
  runs (`__ow_method`/`__ow_headers`/`__ow_path`/`__ow_query`/`__ow_body` → `Request`, and back to
  `{statusCode, headers, body}`).
- `project.yml` — the function project config, never deployed.
...
`./<provider>` entry is the retired shape growing back. Note what the two remaining shapes have in
common: **neither names a vendor.** A container runs anywhere, and a host that mounts the app is a
host. That is the property to keep.
...
- A request is **bounded by the load balancer**, not by an invocation timeout. Sending to 18k
...
ADR-003, 006, 008, 012, 019, 021, 023 and 026 all cite the function runtime somewhere in their
reasoning, usually by name. They are **not** being rewritten — an ADR is a record of a decision and the forces at the time, and editing
...
- **ADR-021** argued for recording unhandled webhook events partly because *DigitalOcean's activation
  logs cannot be searched or alerted on* — a genuinely DO-specific observability gap. Container logs
  can be searched. The decision stands on its remaining reasons —
...
- **ADR-023** noted there is *no socket to read* because the function entrypoint rebuilt a `Request`
  from OpenWhisk's `__ow_headers`. A container has a socket. The decision is unchanged, on a better reason: a
...
- **A major version.** Anyone importing `cablegram/function` breaks — which is, as far as we know,
  nobody, since the target it served could not reach a database.
...
- **Re-adding a serverless target later is real work**, not a revert: a new adapter, a new export
  (a `feat:`), and a fresh answer to how it reaches the database — an answer that may well exist on a
  provider whose functions attach to a private network. That cost is the point: the previous
  arrangement let an unverified target look supported for two weeks.
...
- [ADR-009](ADR-009-deployment-digitalocean-functions.md) — superseded *as a target choice*; its
  runtime constraints stand (§3). Its filename still names the vendor, as the record of that decision
  should.
